...
def main():
    ...
    def collect_form_data_paths(form_xml_path):
        ...
        for m in re.finditer(
            r"<DataPath>[^<]*\b\u041e\u0431\u044a\u0435\u043a\u0442\.(\w+(?:\.\w+)*)</DataPath>",
            content,
        ):
            path = m.group(1)
            segments = path.split(".")
        ...
        for m in re.finditer(
            r"<TitleDataPath>[^<]*\b\u041e\u0431\u044a\u0435\u043a\u0442\.(\w+(?:\.\w+)*)</TitleDataPath>",
            content,
        ):
            path = m.group(1)
            segments = path.split(".")
    ...
    def borrow_main_attribute(type_name, obj_name, form_name, mode):
        ...
        for ts in src_ts:
            ts_xml = build_adopted_tabular_section_xml(
                ts["Name"], ts["Uuid"], ts["GeneratedTypes"], ts["Attributes"], "\t\t\t"
            )
        ...
        if extra_props:
            ...
            obj_content = obj_content.replace(
                "</ExtendedConfigurationObject>", f"</ExtendedConfigurationObject>{props_xml}"
            )
            ...
            if real_deep:
                ...
                for attr_name, sub_attr_names in deep_by_attr.items():
                    ...
                    if not test_object_borrowed(target_type_name, target_obj_name):
                        ...
                        t_borrowed_xml = build_borrowed_object_xml(
                            target_type_name, target_obj_name, t_src["Uuid"], t_src["Properties"]
                        )
                    ...
                    if sub_resolved["Attributes"]:
                        ...
                        for srt in sub_ref_types:
                            ...
                            if test_object_borrowed(srt["TypeName"], srt["ObjName"]):
                                continue
                            s_src_file = os.path.join(
                                cfg_dir,
                                CHILD_TYPE_DIR_MAP[srt["TypeName"]],
                                f"{srt['ObjName']}.xml",
                            )
                            if not os.path.isfile(s_src_file):
                                continue
                            s_src = read_source_object(srt["TypeName"], srt["ObjName"])
                            s_borrowed_xml = build_borrowed_object_xml(
                                srt["TypeName"], srt["ObjName"], s_src["Uuid"], s_src["Properties"]
                            )
    ...
    def borrow_form(type_name, obj_name, form_name, borrow_main_attr=False):
        ...
        if src_child_items is not None:
            ...
            child_items_xml = re.sub(
                r"<CommandName>[^<]*</CommandName>", "<CommandName>0</CommandName>", child_items_xml
            )
            ...
            if borrow_main_attr:
                ...
                child_items_xml = re.sub(
                    r"\s*<TitleDataPath>(?!\u041e\u0431\u044a\u0435\u043a\u0442\.)[^<]*</TitleDataPath>",
                    "",
                    child_items_xml,
                )
            ...
            child_items_xml = re.sub(
                r"\s*<TypeLink>\s*<xr:DataPath>Items\.[^<]*</xr:DataPath>.*?</TypeLink>",
                "",
                child_items_xml,
                flags=re.DOTALL,
            )
            ...
            for pic_name in referenced_pictures:
                if not test_object_borrowed("CommonPicture", pic_name):
                    ...
                    if os.path.isfile(pic_src_file):
                        ...
                        borrowed_xml = build_borrowed_object_xml(
                            "CommonPicture", pic_name, src["Uuid"], src["Properties"]
                        )
            ...
            pic_block_pattern = re.compile(
                r"\s*<Picture>\s*<xr:Ref>CommonPicture\.(\w+)</xr:Ref>.*?</Picture>", re.DOTALL
            )
            ...
            child_items_xml = re.sub(
                r"\s*<Picture>\s*<xr:Ref>StdPicture\.(?!Print\b)\w+</xr:Ref>.*?</Picture>",
                "",
                child_items_xml,
                flags=re.DOTALL,
            )
            ...
            for style_name in referenced_styles:
                if not test_object_borrowed("StyleItem", style_name):
                    ...
                    if os.path.isfile(style_src_file):
                        ...
                        borrowed_xml = build_borrowed_object_xml(
                            "StyleItem", style_name, src["Uuid"], src["Properties"]
                        )
            ...
            for enum_name, needed_values in referenced_enum_values.items():
                if not test_object_borrowed("Enum", enum_name):
                    ...
                    if os.path.isfile(enum_src_file):
                        ...
                        borrowed_xml = build_borrowed_object_xml(
                            "Enum", enum_name, src_obj["Uuid"], src_obj["Properties"]
                        )
                        if ev_xmls:
                            ev_block = "\n".join(ev_xmls)
                            borrowed_xml = borrowed_xml.replace(
                                "<ChildObjects/>",
                                f"<ChildObjects>\n{ev_block}\n\t\t</ChildObjects>",
                            )
    ...
    for item in items:
        ...
        if dot_idx < 1:
            print(
                f"Invalid format '{item}', expected 'Type.Name' or 'Type.Name.Form.FormName'",
                file=sys.stderr,
            )
            sys.exit(1)
...
class CfeBorrowError(Exception):
    """Raised when borrowing objects into an extension fails."""


def borrow_objects(extension_path, config_path, objects, borrow_main_attribute=None):
    """Borrow one or more objects from configuration into extension.

    objects: string (``;;``-separated) or sequence of object specs
             (e.g. ``Catalog.Контрагенты`` or ``Catalog.X.Form.Y``).
    borrow_main_attribute: None, ``Form``, or ``All``.
    """
    if isinstance(objects, (list, tuple, set)):
        object_arg = " ;; ".join(objects)
    else:
        object_arg = objects
    ...
    if borrow_main_attribute:
        argv.append("-BorrowMainAttribute")
