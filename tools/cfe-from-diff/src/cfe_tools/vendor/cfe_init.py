...
class CfeInitError(Exception):
    """Raised when extension scaffold cannot be created."""
...
def create_extension(
    ...
    version="",
    vendor="",
    compatibility_mode="Version8_3_24",
    config_path=None,
    no_role=True,
):
    """Create a minimal Designer XML extension scaffold.

    Returns absolute path to the extension directory.
    """
    synonym = synonym if synonym else name
    name_prefix = name_prefix if name_prefix else f"{name}_"
    purpose = purpose
    ...
    if os.path.exists(cfg_file):
        raise CfeInitError(f"Configuration.xml already exists: {cfg_file}")
    ...
    if config_path:
        ...
        if os.path.isdir(config_path):
            ...
            if os.path.exists(candidate):
                ...
                raise CfeInitError(f"No Configuration.xml in config directory: {config_path}")
        if not os.path.exists(config_path):
            raise CfeInitError(f"Config file not found: {config_path}")
        ...
        print(
            "[WARN] Language ExtendedConfigurationObject set to zeros. Use config_path to auto-resolve from base config, or fix manually before loading."
        )
...
def main():
    ...
    try:
        create_extension(
            ...
            vendor=args.Vendor,
            compatibility_mode=args.CompatibilityMode,
            config_path=args.ConfigPath,
            no_role=args.NoRole,
        )
    except CfeInitError as exc:
